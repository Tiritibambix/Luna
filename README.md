...
> **Fork notice — differences from upstream (as of 2026-06-28)**
>
...
> - **Add an event "Location" field**, like Google Calendar's `location`. Read/written through CalDAV and iCal's `LOCATION` property (RFC 5545) and through Google Calendar's `location` field, editable in the event modal, and shown with a map-pin icon on calendar events; read-only iCal sources get a local override the same way `desc` already did. See `GetLocation`/`SetLocation` on the `Event` interface in [backend/src/types/primitives_events.go](backend/src/types/primitives_events.go), the iCal parsing in [backend/src/protocols/internal/common.go](backend/src/protocols/internal/common.go), and the form field in [frontend/src/components/modals/EventModal.svelte](frontend/src/components/modals/EventModal.svelte). Adding the `event_overrides.location` column required a real migration, since the `0.1.0` migration only runs on fresh installs — see [backend/src/db/internal/migrations/internal/versions/0.1.1.go](backend/src/db/internal/migrations/internal/versions/0.1.1.go). Dev hint: whenever you register a new migration, bump `backend/VERSION` (and `frontend/VERSION`, to keep the compatibility check in [frontend/src/lib/common/version.ts](frontend/src/lib/common/version.ts) happy) or existing databases will never pick it up.
